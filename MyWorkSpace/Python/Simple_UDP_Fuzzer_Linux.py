...
def usage():
	print >>sys.stderr, "[-] Simple_UDP_Fuzzer.py [-h|-p|-l] -s src_ip -d dst_ip -S src_ip -D dst_port"
	...
	print >>sys.stderr, "                     -l length    : Max length of packet's data"
	sys.exit(-1)
...
def fuzzing(netinfo):
	...
	while True:
		...
		result = s.sendto(packet, ('ens33', socket.SOCK_RAW))
		sleep(1)
